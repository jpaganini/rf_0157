...
##3.1  Calculate jaccard distances distributions
...
#3.Calculate Jaccard distances using the veggdist
...
## 3.2 Explore potential different distance cutoffs for Jaccard distances
...
## 3.3 Select a jaccard distance of 1 as cut-off
This is, features are clustered only when they co-occur in all isolates.
```{r fig.width=20, fig.height=20, dpi=300}
#keep only connections in which the weight is 1
...
#4. SHAP VALUES
## 4.1 Create network using shap values.
...
##4.2 Load the SHAP values importances and feature annoations
```{r}
...
## 4.6 - Supplementary DAtaset 3 and rename before plotting
...
#These 3 tables, pulled together and re-format constitute supplementary dataset 3
#write.table(hus_absolute_shap_cluster,"../results/99_supplementary_dataset/hus_absolute_shap_cluster.tsv", sep='\t', row.names = FALSE, quote = FALSE, col.names = TRUE)
#write.table(bd_absolute_shap_cluster,"../results/99_supplementary_dataset/bd_absolute_shap_cluster.tsv", sep='\t', row.names = FALSE, quote = FALSE, col.names = TRUE)
#write.table(d_absolute_shap_cluster,"../results/99_supplementary_dataset/d_absolute_shap_cluster.tsv", sep='\t', row.names = FALSE, quote = FALSE, col.names = TRUE)
...
##5.1 Filter the top 10 clusters for each outcome
...
## 5.3 Supplementary Figures S8 -- SHAP BARPLOT -- ABSENCE ALL
...
```
## 5.4 Supplementary Figures S6 and S7 - SHAP BARPLOT - PRESENCE BD and D
...
```
## 5.5 Figure 2 A - SHAP BARLOPT - HUS PRESENCE
...
feature_importances_bd_sub<-ggplot(hus_presence_shap_barplot_data_sub, aes(x=reorder(Cluster_ann,-Cluster_Average_SHAP), y=Cluster_Average_SHAP, fill=Phage)) + 
  geom_bar(stat='identity', position='stack', alpha=0.8) +
  theme_bw()+
  scale_fill_manual(values = c('#fde725',"#21918c","#365c8d",'#440154'))+
  theme(plot.title = element_text(size=12,hjust = 0.5,face='bold'),
...
## 5.6 Plot the features-clusers network
### Figure 2, Sup Figure S6 and S7 - PRESENCE CLUSTERS
...
### Sup Figure S8 - ABSENCE CLUSTERS
...
#NO CLUSTERS TO PLOT, JUST INDICVIDUAL FEATURES

# #Assign names to the clusters=====
# #1. Create a dataframe for assigning names to vertexes
# cluster_ann_hus_df<-hus_absolute_shap_cluster[!is.na(hus_absolute_shap_cluster$Cluster_id),c(1,3,5)]
# cluster_ann_hus_df$ANN <- sub("^Feature.*? - (.*?) \\[.*?\\]$", "\\1", cluster_ann_hus_df$ANN)
# #2. Pick a random feature from each unique annotation
# cluster_ann_hus_df<-cluster_ann_hus_df[,c(2,3)] %>% group_by(Cluster_id, ANN) %>% unique()
# 
...
## 6.6 Create the final dataframe for annotating the tree
...
## 6.7 SUPPLEMENTARY FIGURE S12 - Plot the distribution of features as heat maps
...
#7. Analysis of the Upstream regino of KilR.
